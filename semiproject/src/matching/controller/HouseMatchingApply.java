 ...
 */
@WebServlet("/housematchingApply.ma")
public class HouseMatchingApply extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HouseMatchingApply() {
        super();
...
/*		            // 타이틀 이미지가 originFiles에서의 마지막 인덱스이기 때문에 다음과 같이 조건을 준 다음에 level을 0으로 지정
				...
				String caredateStr = multiRequest.getParameter("caredate");
				int petno1 = Integer.valueOf(multiRequest.getParameter("petno1"));
				int petno2 = Integer.valueOf(multiRequest.getParameter("petno2"));
				...
				if(petno1==0){
					if(petno2==0){
						...
						countpet=1;
						result3 = new PetService().updatePet(petno2);
					}
				}else{
					if(petno2==0){
						countpet=1;
						result2 = new PetService().updatePet(petno1);
					}else{
						countpet=2;
						result2 = new PetService().updatePet(petno1);
						result3 = new PetService().updatePet(petno2);
				...
				java.sql.Date caredate = java.sql.Date.valueOf(caredateStr);
				
				
		/*		java.sql.Date caredate = null;
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
				try {
					
					java.util.Date date = sdf.parse(caredateStr);
					caredate = new java.sql.Date(date.getTime());  
				...
				int result = new MatchingService().insertHouseMatching(userId,price,wanttosay,specialnote,caredate,countpet,matImgName);
		         
				if(result>0){
					response.sendRedirect("select.hm");
				}else{
