...
<body>
	...
	<div id="outer">
		...
		<h1 class="text-info">펫시터 지원서</h1><br><br>
	...
	1. 목적 : 지원자 개인 식별, 지원의사 확인 2. 항목 : 지원자 성명, 휴대폰번호 및 해당 신청서에서 수집된 정보 3. 보유기간 : 해당 지원자 모집 종료 후 1년간 보유, 이용됩니다. 4. 동의를 거부할 권리 및 동의 거부에 따른 불이익 : 지원자는 위 내용에 대해서 동의를 하지 않을 권리가 있으며 지원자가 개인정보의 삭제를 요청하는 경우 지체없이 이를 삭제합니다.
		...
		<hr>
		<h4 class="text-info">주거 공간</h4>
	    <input type="radio" id="apart" name="residence" value="아파트" checked>
	    <label for="apart">아파트</label>&nbsp;&nbsp;
	    <input type="radio" id="villa" name="residence" value="빌라">
	    <label for="villa">빌라</label>&nbsp;&nbsp;
	    <input type="radio" id="detachedHouse" name="residence" value="단독 주택">
	    <label for="detachedHouse">단독 주택</label>&nbsp;&nbsp;
	    <input type="radio" id="etc" name="residence" value="기타">
	    <label for="etc">기타</label>
	    <hr>
		<h4 class="text-info">직업</h4>
	    <input type="radio" id="jobSeekers" name="job" value="취업준비생" checked>
	    <label for="jobSeekers">취업준비생</label>&nbsp;&nbsp;
	    <input type="radio" id="houseWife" name="job" value="주부">
	    <label for="houseWife">주부</label>&nbsp;&nbsp;
	    <input type="radio" id="officeWorkers" name="job" value="직장인">
	    <label for="officeWorkers">직장인</label>&nbsp;&nbsp;
	    <input type="radio" id="etc" name="job" value="기타">
	    <label for="etc">기타</label>
	    <hr>
	...
	<script type="text/javascript">
 		$(document).ready(function(){
			$("#fnSubmit").click(function(){
				...
				if(!chk1){
					alert("첫번째 이용약관에 동의해주세요");
					$("#agree1").focus();
					return false;
				}else if(!chk2){
					alert("두번째 이용약관에 동의해주세요");
					$("#agree2").focus();
					return false;
				}
